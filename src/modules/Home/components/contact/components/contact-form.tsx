    ...
    <FormProvider {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='w-full'>
        <div className='grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-3 max-[640px]:grid-cols-1'>
          ...
          <CommonSelect name='country' placeholder='Country *' options={COUNTRIES} />
        </div>
        <CommonTextArea name='message' placeholder='Message' rows={4} className='mt-3 h-[190px] resize-none' />

        <CommonButton
          type='submit'
          className='!pl-4 !pr-3 hover:!bg-[#0B1D4E] hover:!text-white mt-6 h-11 min-w-[178px] gap-1 bg-transparent text-white'
